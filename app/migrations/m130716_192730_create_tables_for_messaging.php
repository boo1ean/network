...
class m130716_192730_create_tables_for_messaging extends \yii\db\Migration
{
	public function up()
	{
        ...
        // Add foreign key for conversation_id in user_conversations table
        $this->db->createCommand()
            ->addForeignKey('conversation_id_fk_in_user_conversations', 'user_conversations', 'conversation_id', 'conversations', 'id')
            ->execute();
        ...
        // Add foreign key for conversation_id in messages table
        $this->db->createCommand()
            ->addForeignKey('conversation_id_fk_in_messages', 'messages', 'conversation_id', 'conversations', 'id')
            ->execute();
        */
	...
	{
        /*
        // Drop user_id foreign key in messages table
        $this->db->createCommand()
            ->dropForeignKey('user_id_fk_in_messages', 'messages')
            ->execute();
        // Drop conversation_id foreign key in messages table
        $this->db->createCommand()
        ...
        */
        ...
        $this->db->createCommand()
            ...
            ->execute();
        /*
        // Drop user_id foreign key in user_conversations table
        $this->db->createCommand()
            ->dropForeignKey('user_id_fk_in_user_conversations', 'user_conversations')
            ->execute();
        // Drop conversation_id foreign key in user_conversations table
        $this->db->createCommand()
